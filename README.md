...
2026-09-23 软件推进：已有普通 U 形车位线候选、可选固定相机地面投影、本地图片/录像分析及数据集泄漏检查；新增模拟末段倒车、重新规划出库/阻挡拒绝和随机几何测试。运行入口生成 `results/阶段一软件实验.html`；[使用说明与实拍采样办法](offline/README.md)。全部保持实验状态，无实车或实拍识别效果承诺。

已有纯 C motor/encoder/PI/command/telemetry、安全编排与测试，独立单电机 STM32 候选、开源规划适配、电脑端仿真/故障注入、标定/日志/回放工具。

尚无实车完整自主泊车验收。普通线候选尚缺实拍/语义与占用验证；随机找位、真实出库/相机闭环、正式 RDK 通信和跨场地能力仍未完成。仿真用假设模型与真值位姿；AprilTag 代码仅隔离实验。编译、主机测试和仿真不等于上板安全。
